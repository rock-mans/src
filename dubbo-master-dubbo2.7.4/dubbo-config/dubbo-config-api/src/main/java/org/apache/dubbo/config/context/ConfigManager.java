 ...
 *
 *  此处定义的属性与ReferenceConfig / ServiceConfig中的属性重复，
 *  此处的属性当前仅用于重复检查，但仍未在导出/引用过程中使用。
 *  也许我们可以删除ReferenceConfig / ServiceConfig中的属性定义，而只保留setXxxConfig（）作为入口。
 *  所有内部工作流程都可以依赖ConfigManager。
 */
