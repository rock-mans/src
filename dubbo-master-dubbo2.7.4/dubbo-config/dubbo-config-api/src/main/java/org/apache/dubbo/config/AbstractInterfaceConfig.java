...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
    ...
    protected void checkRegistry() {
        // 从当前 ReferenceConfig中加载 config信
        loadRegistriesFromBackwardConfig();
        //
        convertRegistryIdsToRegistries();

        //<dubbo:registry address="zookeeper://192.168.6.90:2181" protocol="zookeeper" port="2181" valid="true" zookeeperProtocol="true" id="org.apache.dubbo.config.RegistryConfig" prefix="dubbo.registries." />
        //registries
    ...
    private void prepareEnvironment() {
        if (configCenter.isValid()) {
            ...
            //url====zookeeper://192.168.6.90:2181/ConfigCenterConfig?config.check=true&config.config-file=dubbo.properties&config.group=dubbo&config.highest-priority=false&config.namespace=dubbo&config.timeout=3000
            // 加载配置中心
        ...
        if (CollectionUtils.isNotEmpty(registries)) {
            for (RegistryConfig config : registries) {
                ...
                if (!RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(address)) {
                    ...
                    // 解析得到 URL 列表，address 可能包含多个注册中心ip，
                    // 因此解析得到的是一个 URL 列表
                    List<URL> urls = UrlUtils.parseURLs(address, map);
                    for (URL url : urls) {
                        url = URLBuilder.from(url)
                                .addParameter(REGISTRY_KEY, url.getProtocol())//添加registry=zookeeper
                                .setProtocol(REGISTRY_PROTOCOL)//zookeeper://192.168.6.90:2181---->registry://192.168.6.90:2181
                                .build();
    ...
    private void useRegistryForConfigIfNecessary() {
        registries.stream().filter(RegistryConfig::isZookeeperProtocol).findFirst().ifPresent(rc -> {
            ...
            //我们使用DynamicConfiguration的加载状态来确定是否已启动ConfigCenter
            ...
            Environment.getInstance().getDynamicConfiguration().orElseGet(() -> {
                ...
                //cc=<dubbo:config-center address="zookeeper://192.168.6.90:2181" protocol="zookeeper" valid="true" namespace="dubbo" highestPriority="false" check="true" timeout="3000" group="dubbo" configFile="dubbo.properties" prefix="dubbo.config-center" />
                setConfigCenter(cc);
                //启动(连接)配置中心
                startConfigCenter();
                return null;
