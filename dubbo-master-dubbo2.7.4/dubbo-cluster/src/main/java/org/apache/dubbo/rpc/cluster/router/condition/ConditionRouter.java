...
public class ConditionRouter extends AbstractRouter {
    ...
    public ConditionRouter(URL url) {
        logger.info("555url:"+url);
        this.url = url;
        // 获取 priority 和 force 配置
        this.priority = url.getParameter(PRIORITY_KEY, 0);
        this.force = url.getParameter(FORCE_KEY, false);
        this.enabled = url.getParameter(ENABLED_KEY, true);
        init(url.getParameterAndDecoded(RULE_KEY));// url.getParameterAndDecoded(RULE_KEY) 获取路由规则
    }

    public void init(String rule) {
        logger.info("555rule:"+rule);
        ...
        try {
            ...
            logger.info("555whenRule:"+whenRule);
            logger.info("555thenRule:"+thenRule);
    ...
    private static Map<String, MatchPair> parseRule(String rule)
        ...
        // 通过正则表达式匹配路由规则，ROUTE_PATTERN = ([&!=,]*)\s*([^&!=,\s]+)    [&!=,]*)\s*([^&!=,\s]+)
...
//        return true;
    ...
    //第二个和第三个参数来自 matchThen 方法的参数列表，这两个参数分别为服务消费者 url和服务提供者 url 。????????/**/
