...
final class NettyChannel extends AbstractChannel {
    ...
    public void send(Object message, boolean sent) throws RemotingException {
        ...
        int timeout = 0;
        try {
            //writeAndFlush等价于先将数据写到netty的缓冲区，
            //再将netty缓冲区中的数据写到Socket缓冲区中，写的过程与并发编程类似，用自旋锁保证写成功
            //执行writeAndFlush并没有真正发送出去，还要执行编码工作，：
            //InternalEncoder#encode->DubboCountCodec#encode->ExchangeCodec#encode->ExchangeCodec#encodeRequest
            ChannelFuture future = channel.writeAndFlush(message);
            logger.info("channel.writeAndFlush............["+message);
            if (sent) {
                // wait timeout ms
