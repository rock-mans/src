...
public class DubboProtocol extends AbstractProtocol {
    ...
    Invoker<?> getInvoker(Channel channel, Invocation inv) throws RemotingException {
        ...
        // 获取 Invoker 对象，并返回 AbstractProxyInvoker
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        ...
        // 将 <key, exporter> 键值对放入缓存中，后面调用链会使用的
        ...
        openServer(url);
        // 优化序列化
        //将指定的序列化类加载到缓存，通常配置kryo, fst序列化，不配置，默认用hessian2序列化
        ...
        if (isServer) {
            ...
            if (server == null) {
                synchronized (this) {
                    ...
                    if (server == null) {
                        //根据URL创建ExchangeServer
                        serverMap.put(key, createServer(url));
                    }
                }
            } else {
                // 服务器已创建，则根据 url 中的配置重置服务器
                // 如果存在<dubbo:service interface="org.apache.dubbo.demo.DemoService" ref="demoService"/>的情况下
                // 会进入此分支
                server.reset(url);
            }
        }
